...
/**
 * Sample output schema for the SDK-backed incident-triage pact.
 *
 * Mirrors the schema in the manual-loop sibling sample
 * (`samples/cortex-incident-triage-agent`). Kept identical so the two
 * samples can be diffed cleanly when learning the difference between
 * the manual loop (`pacta-provider-anthropic`) and the SDK loop
 * (`pacta-provider-claude-agent-sdk`).
 */
