...
## Run 3 — SmolLM2-135M, 3000 steps, 10K CAUSALLY CONSISTENT corpus

**Date:** 2026-03-28
**Model:** HuggingFaceTB/SmolLM2-135M-Instruct (134.5M params)
**Corpus:** 10,000 train + 2,500 holdout — generated by `augment-corpus-v2.py`
**Fix:** All output fields (anomalies, restrictedActions, forceReplan, escalation) are
deterministic functions of input signal values. No random field assignment.

### Training Metrics

| Metric | Run 1 | Run 2 | Run 3 | Delta (1→3) |
|--------|-------|-------|-------|-------------|
| Corpus size | 4,000 | 4,000 | **10,000** | +6,000 |
| Steps | 1,000 | 5,000 | 3,000 | +2,000 |
| Final loss | 0.434 | 0.286 | **0.277** | -0.157 |
| Training time | 3.8 min | 16.8 min | **11.4 min** | +7.6 min |
| Peak VRAM | 2,950 MB | 2,950 MB | **2,950 MB** | same |

### Evaluation Results (2,500 holdout)

| Metric | Target | Run 1 | Run 2 | Run 3 | Status |
|--------|--------|-------|-------|-------|--------|
| Parse accuracy | >= 95% | 100% | 100% | **100%** | PASS |
| Semantic accuracy | >= 85% | 39.2% | 39.3% | **98.6%** | PASS |
| Adversarial accuracy | >= 70% | 11.0% | 11.6% | **70.8%** | PASS |
| Confidence mean | -- | 96.0% | 96.2% | **99.97%** | Overconfident |

### Analysis

**The fix was data quality, not model scale or training duration.**

Run 2 proved that 5x more training steps (2→10 epochs on the same data) doesn't help —
the model plateaued at 39% semantic accuracy. Run 3 proved that causally consistent data
is the key: with the same model (135M params), fewer steps (3000 vs 5000), but a corpus
where every output field is a deterministic function of input signals, semantic accuracy
jumped from 39% → 98.6%.

The specific fix in `augment-corpus-v2.py`:
- `restrictedActions` = actions from signals with `unexpectedResult=true` (not random)
- `forceReplan` = true when >= 2 anomalies (not random)
- `escalation` = present when forceReplan is true (not random)
- Anomaly types derive from signal content: low confidence → `low-confidence`,
  unexpected result → `unexpected-result`, both → `compound`

**Overconfidence persists (99.97% mean).** The model is even more confident in Run 3 —
but now it's mostly correct (98.6%), so the high confidence is partially justified.
Temperature scaling calibration is still needed for the 1.4% error cases.

### Gate 3 Verdict

**ALL THREE METRICS PASS.** Gate 3 is cleared. Proceed to Phase 4 (integration).
...
| | Run 1 | Run 2 | Run 3 |
|---|---|---|---|
| **Corpus** | 4K random | 4K random | 10K causally consistent |
| **Steps** | 1,000 | 5,000 | 3,000 |
| **Parse accuracy** | 100% | 100% | **100%** |
| **Semantic accuracy** | 39.2% | 39.3% | **98.6%** |
| **Adversarial** | 11.0% | 11.6% | **70.8%** |
| **Training time** | 3.8 min | 16.8 min | 11.4 min |
| **VRAM** | 2.95 GB | 2.95 GB | 2.95 GB |

## What's Validated

- RFC 002 core thesis: SLMs CAN learn typed DSLs (100% parse accuracy, all 3 runs)
- 135M params is sufficient — no escalation to 360M or 500M needed
- Training infrastructure works (11 min on single RTX 2080 Ti)
- VRAM headroom is massive (2.95GB / 11GB — room for 3-4x larger model)
- The peggy grammar is fully learnable by small models
- **Data quality > training duration** — the critical lesson for RFC 002
- Overconfidence risk is real — calibration is mandatory before integration

## What Remains (Gate 4)

1. Temperature scaling calibration (ECE target: <= 0.15)
2. ONNX export + accuracy validation (target: <= 2% delta)
3. HTTP bridge inference server
4. SLMProviderAdapter cycle benchmark (10 tasks, cost reduction >= 30%)
