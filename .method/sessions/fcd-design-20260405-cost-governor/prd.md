...
**Types imported from `@method/types`:** `InvocationSignature`, `CostBand`.

```typescript
// @method/types (L0) — consumed by bridge + pacta + methodts
...
**Types imported from `@method/types`:** `ProviderClass`, `AccountUtilization`, `AccountCapacity`.
...
**Types imported from `@method/types`:** `InvocationSignature`, `ProviderClass`, `AccountId`.
...
import type { ProviderClass, AccountCapacity, AccountId } from '@method/types';
...
Located in `@method/types` (L0 package) — consumed by bridge (L4), pacta (L3), methodts (L2):

| Entity | Location |
|---|---|
| `InvocationSignature` | `@method/types` |
| `ProviderClass` | `@method/types` |
| `CostBand` | `@method/types` |
| `AccountCapacity` | `@method/types` |
| `AccountUtilization` | `@method/types` |
| `AccountId`, `SlotId` (branded) | `@method/types` |
...
**`shared/canonical-types/` directory is NOT created** — it doesn't fit the bridge's existing `@method/types` + `src/ports/` convention.
...
> **Layer reminder:** cost-governor is an **L4 bridge domain** (all bridge domains are L4). Layer stack: L0 `@method/types` → L2 `@method/methodts` → L3 `@method/mcp`/`@method/pacta` → L4 `@method/bridge`. The PRD's earlier "cost-governor at L2" framing was incorrect.
...
Imports allowed: `@method/types` (L0), `@method/methodts` (L2), `@method/pacta` (L3), sibling L4 domains via `packages/bridge/src/ports/` only.
...
| G-LAYER | cost-governor (L4) imports only from L0 `@method/types`, L2 `@method/methodts`, L3 `@method/pacta`, sibling L4 via `ports/` | `G-LAYER: cost-governor L4 dependencies` |
| G-ENTITY | InvocationSignature/ProviderClass/CostBand defined only in `@method/types` | `G-ENTITY: canonical types single source` |
...
**Deliverables:**
- Canonical types in `@method/types` (L0): `InvocationSignature`, `ProviderClass`, `CostBand`, `AccountCapacity`, `AccountUtilization`, `AccountId`, `SlotId` branded types.
...
1. ~~`InvocationSignature` location?~~ **Resolved (fcd-review):** lives in `@method/types` (L0 package), consumed by bridge + pacta + methodts. The `shared/canonical-types/` directory proposed in the first draft does not exist and conflicts with the established `@method/types` convention.
