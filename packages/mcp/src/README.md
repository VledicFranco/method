...
L3 package. Thin MCP (Model Context Protocol) server that exposes `@methodts/methodts` methodology tools and `@fractal-co-design/fca-index` context tools to Claude agents via the standard MCP stdio transport.
...
MCP handlers are thin wrappers only: parse input → call port → format output. No domain logic in handlers. All business logic lives in `@methodts/methodts`, `@fractal-co-design/fca-index`, or `@methodts/bridge`.
