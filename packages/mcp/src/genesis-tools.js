 ...
 */
export function enforceGenesisPrivilege(ctx) {
    if (ctx.project_id !== 'root') {
        const errorMsg = ctx.project_id
...
export const genesisToolDefinitions = [
    {
        name: 'project_list',
        description: 'List all discovered projects with metadata (id, name, description, installed_methodologies, status)',
        inputSchema: {
            type: 'object',
            properties: {},
        },
    ...
    },
    {
        ...
        description: 'Read project events with cursor-based pagination. Filter by project_id to read events for that project only.',
        inputSchema: {
            type: 'object',
            properties: {
                project_id: {
    ...
    {
        name: 'genesis_report',
        description: 'Report findings to the human operator. SECURITY: Only callable by Genesis session (project_id="root"). Non-Genesis sessions will receive 403 Forbidden.',
        inputSchema: {
            type: 'object',
            properties: {
                message: {
 ...
 */
export async function validateGenesisToolInput(toolName, toolInput, sessionCtx) {
    try {
        let validated;
        switch (toolName) {
            case 'project_list':
...
//# sourceMappingURL=genesis-tools.js.map