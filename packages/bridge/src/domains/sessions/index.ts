 ...
 * machinery moved to `@methodts/runtime/sessions`. Bridge keeps only
 * Fastify routes + Node-specific persistence / transcript / worktree
 * helpers.
 *
 * For external consumers, import engine surface directly from
 * `@methodts/runtime/sessions` rather than through this barrel.
 */
...
export { createPool } from '@methodts/runtime/sessions';
export type {
  SessionPool,
  SessionSnapshot,
  SessionBudget,
...
export type { ChannelMessage, SessionChannels } from '@methodts/runtime/sessions';
