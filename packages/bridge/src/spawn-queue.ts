...
export class SpawnQueue {
  ...
  /**
   * Enqueue a spawn operation. The returned promise resolves with the
   * spawn result once the operation completes (after waiting for the
   * minimum gap if needed).
   */
  ...
  /** Number of items waiting (not yet started). */
  get pending(): number {
    return this.queue.length;
  }

  /** Timestamp of the last completed spawn. */
