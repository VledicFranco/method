...
/**
 * Strategy DAG Retrospective Generator — generates mandatory retros.
 *
 * Migrated from bridge retro-generator.ts (PRD 017 Phase 1d).
 * This is pure logic only — filesystem operations (save) remain in the bridge.
 *
 * Captures timing, cost, gate results, oversight events, and artifacts
 * produced during execution.
 *
 * @see PRD 017 — Strategy Pipelines (retrospective generation)
 */
...
export function generateRetro(
  ...
  const sequentialTime = Object.values(result.node_results).reduce(
    (sum, nr) => sum + nr.duration_ms,
    0,
  );
  const speedupRatio =
    result.duration_ms > 0 ? sequentialTime / result.duration_ms : 1.0;

  // Count completed / failed nodes
  const nodeResults = Object.values(result.node_results);
  const nodesCompleted = nodeResults.filter(
    ...
    if (nr.retries > 0) {
      ...
      if (nodeObj) {
        for (const gate of nodeObj.gates) {
          const gateResults = nr.gate_results.filter((gr) =>
            gr.gate_id.startsWith(nr.node_id + ":"),
          );
          if (gateResults.length > 0) {
            ...
            const entry = {
              ...
              final: (finalResult.passed ? "passed" : "failed") as
                | "passed"
                | "failed",
            };
            retries.push(entry);
...
export function computeCriticalPath(
  ...
  for (const node of dag.nodes) {
    ...
    for (const dep of node.depends_on) {
      if (!dependents.has(dep as string))
        dependents.set(dep as string, []);
      dependents.get(dep as string)!.push(node.id);
    }
  }
  ...
  while (queue.length > 0) {
    ...
    for (const dep of node.depends_on) {
      const depEct = ect.get(dep as string) ?? 0;
      if (depEct > earliestStart) {
        earliestStart = depEct;
        bestPredecessor = dep as string;
      }
    }
